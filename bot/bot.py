...
async def genkey(ctx: commands.Context, member: discord.Member = None, duration: str = "lifetime", *, note: str = ""):
    ...
    # Send key to the user via DM
    try:
        dm_embed = discord.Embed(
            title="🔑 Your License Key",
            description=f"Your license key has been generated by **{ctx.guild.name}**.",
            color=discord.Color.gold(),
            timestamp=datetime.now(tz=timezone.utc),
        )
        dm_embed.add_field(name="Key",     value=f"```{key_str}```", inline=False)
        dm_embed.add_field(name="Expires", value=expires, inline=True)
        dm_embed.add_field(name="Duration", value=duration, inline=True)
        dm_embed.set_footer(text="Keep this key private. Do not share it with anyone.")
        await member.send(embed=dm_embed)
        dm_status = f"DM sent to {member.mention} ✅"
    except discord.Forbidden:
        dm_status = f"⚠️ Could not DM {member.mention} (DMs disabled). Key: `{key_str}`"
...
async def on_command_error(ctx: commands.Context, error):
    ...
    if isinstance(error, commands.MemberNotFound):
        await ctx.send(
            embed=discord.Embed(description="❌ Member not found. Mention a valid server member.", color=discord.Color.red())
        )
        return
    logger.error("Unhandled command error: %s", error, exc_info=True)
    await ctx.send(
        embed=discord.Embed(description=f"❌ Unexpected error: `{error}`", color=discord.Color.red())
    )
